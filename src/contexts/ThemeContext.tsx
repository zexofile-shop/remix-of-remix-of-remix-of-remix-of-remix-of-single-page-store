...
function createSnowfall(): () => void {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    flake.style.cssText = `position:absolute;top:-10px;left:${Math.random()*100}%;width:${size}px;height:${size}px;background:radial-gradient(circle, rgba(255,255,255,0.9), rgba(200,220,255,0.4));border-radius:50%;--drift:${drift}px;animation:snowDrift ${5+Math.random()*8}s ${Math.random()*5}s linear infinite;filter:blur(${Math.random() > 0.7 ? 1 : 0}px);`;
...
function createRain(): () => void {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    drop.style.cssText = `position:absolute;top:-20px;left:${Math.random()*100}%;width:1.5px;height:${15+Math.random()*20}px;background:linear-gradient(transparent, hsl(200 70% 65% / 0.5));border-radius:999px;animation:rainDrop ${0.4+Math.random()*0.4}s ${Math.random()*2}s linear infinite;`;
...
function createPetals(): () => void {
  ...
  for (let i = 0; i < 18; i++) {
    ...
    petal.style.cssText = `position:absolute;top:-20px;left:${Math.random()*100}%;width:${size}px;height:${size*1.4}px;background:${color};border-radius:50% 0 50% 0;--sway:${(Math.random()-0.5)*80}px;animation:petalFloat ${6+Math.random()*6}s ${Math.random()*4}s ease-in-out infinite;opacity:0.6;`;
...
function createHearts(): () => void {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    heart.style.cssText = `position:absolute;bottom:-20px;left:${Math.random()*100}%;width:${size}px;height:${size}px;background:hsl(${hue} 80% 60%);transform:rotate(45deg);--rot:${(Math.random()-0.5)*40}deg;animation:heartRise ${8+Math.random()*6}s ${Math.random()*5}s ease-out infinite;`;
...
function createSparkle(config: ThemeConfig): () => void {
  ...
  const interval = setInterval(() => {
    ...
    spark.style.cssText = `position:absolute;left:${Math.random()*100}%;top:${Math.random()*100}%;width:${size}px;height:${size}px;background:radial-gradient(circle, hsl(${config.primaryHsl} / 0.9), transparent);border-radius:50%;box-shadow: 0 0 ${size*2}px hsl(${config.accentHsl} / 0.5);animation:sparklePulse ${1+Math.random()*2}s ease-in-out forwards;`;
...
function createHoli(): () => void {
  const container = getContainer();
  const holiColors = ['hsl(0 80% 60%)', 'hsl(280 80% 60%)', 'hsl(180 80% 50%)', 'hsl(50 90% 55%)', 'hsl(120 70% 50%)', 'hsl(320 80% 60%)', 'hsl(30 90% 55%)', 'hsl(210 80% 60%)'];
  addStyle('theme-anim-style', `
    @keyframes colorBurst { 0% { transform: translateY(0) scale(0.3); opacity: 0.9; } 40% { opacity: 0.6; } 100% { transform: translateY(var(--rise)) scale(1.5); opacity: 0; } }
    @keyframes colorSplash { 0% { transform: scale(0); opacity: 0.8; } 100% { transform: scale(3); opacity: 0; } }
  `);
  const interval = setInterval(() => {
    if (container.children.length > 25) container.removeChild(container.children[0]);
    const color = holiColors[Math.floor(Math.random() * holiColors.length)];
    const puff = document.createElement('div');
    const size = 20 + Math.random() * 40;
    puff.style.cssText = `position:absolute;bottom:0;left:${10+Math.random()*80}%;width:${size}px;height:${size}px;background:radial-gradient(circle, ${color}, transparent 70%);border-radius:50%;--rise:${-(200+Math.random()*400)}px;animation:colorBurst ${2+Math.random()*3}s ease-out forwards;filter:blur(${4+Math.random()*8}px);`;
  ...
  const splashInterval = setInterval(() => {
    ...
    splash.style.cssText = `position:absolute;left:${10+Math.random()*80}%;top:${30+Math.random()*50}%;width:30px;height:30px;background:radial-gradient(circle, ${color}, transparent 60%);border-radius:50%;animation:colorSplash 1.5s ease-out forwards;filter:blur(6px);`;
    container.appendChild(splash);
    setTimeout(() => splash.remove(), 1500);
  }, 800);
  return () => { clearInterval(interval); clearInterval(splashInterval); };
}

function createFireworks(config: ThemeConfig): () => void {
  const container = getContainer();
  const fwColors = [`hsl(${config.primaryHsl})`, `hsl(${config.accentHsl})`, 'hsl(50 90% 60%)', 'hsl(0 80% 60%)', 'hsl(200 80% 60%)'];
  addStyle('theme-anim-style', `
    @keyframes fwRise { 0% { transform: translateY(0); opacity: 1; } 100% { transform: translateY(var(--rise)); opacity: 0.5; } }
  ...
  const interval = setInterval(() => {
    ...
    trail.style.cssText = `position:absolute;bottom:0;left:${x}%;width:3px;height:3px;background:hsl(50 90% 70%);border-radius:50%;--rise:${rise}px;animation:fwRise 0.8s ease-out forwards;box-shadow: 0 0 4px hsl(50 90% 70%);`;
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        p.style.cssText = `position:absolute;left:${x}%;top:calc(100% + ${rise}px);width:${s}px;height:${s}px;background:${color};border-radius:50%;--dx:${Math.cos(angle)*distance}px;--dy:${Math.sin(angle)*distance}px;animation:fwBurst ${0.8+Math.random()*0.5}s ease-out forwards;box-shadow: 0 0 ${s*2}px ${color};`;
...
function createKites(): () => void {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    kite.style.cssText = `position:absolute;left:${10+Math.random()*80}%;top:${10+Math.random()*40}%;width:${size}px;height:${size*1.3}px;background:${color};clip-path: polygon(50% 0%, 100% 40%, 50% 100%, 0% 40%);--tilt:${(Math.random()-0.5)*20}deg;animation:kiteFly ${4+Math.random()*4}s ${Math.random()*2}s ease-in-out infinite;opacity:0.7;`;
    const tail = document.createElement('div');
    tail.style.cssText = `position:absolute;left:50%;bottom:-${size*0.8}px;width:1px;height:${size*0.8}px;background:${color};transform:translateX(-50%);border-bottom-left-radius:50%;border-bottom-right-radius:50%;`;
...
function createSunshine(): () => void {
  const container = getContainer();
  addStyle('theme-anim-style', `
    @keyframes sunPulse { 0%, 100% { opacity: 0.15; transform: scale(1); } 50% { opacity: 0.25; transform: scale(1.05); } }
    @keyframes sunRay { 0%, 100% { opacity: 0.1; } 50% { opacity: 0.2; } }
  `);
  const glow = document.createElement('div');
  glow.style.cssText = `position:absolute;top:-100px;right:-100px;width:400px;height:400px;background:radial-gradient(circle, hsl(40 90% 60% / 0.3), hsl(35 95% 55% / 0.1), transparent 70%);border-radius:50%;animation:sunPulse 6s ease-in-out infinite;`;
  container.appendChild(glow);
  for (let i = 0; i < 6; i++) {
    const ray = document.createElement('div');
    const angle = -30 + i * 15;
    ray.style.cssText = `position:absolute;top:0;right:100px;width:2px;height:300px;background:linear-gradient(hsl(40 90% 60% / 0.15), transparent);transform-origin:top right;transform:rotate(${angle}deg);animation:sunRay ${3+Math.random()*2}s ${Math.random()*2}s ease-in-out infinite;`;
